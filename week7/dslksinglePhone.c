...
#include <stdio_ext.h>
#include <string.h>
#include "dslksinglePhone.h"
#define CONSTANT 100

elementtype input_phone() // Hàm nhập vào 1 phone mới. Bắt lỗi khi giá và size <0
{
    elementtype p;
...
void importFromFileDat(FILE *input1) //HÀm import dữ liệu vào từ file .dat
{
    elementtype *m, p; //Khai báo  con trỏ m, biến p kiểu elementtype
...
void exporttodat()
{
    FILE *output = fopen("phone_export.dat", "wb");
    node *p;
    p = root;
    elementtype *m;
    while (p != NULL)
