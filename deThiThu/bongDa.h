...
treeType search(char *x, treeType root)
{
    ...
    else if (strcasecmp(root->element.doi1, x) == 0)
    { //Neu tim thay
        return root;
    }
    else if (strcasecmp(root->element.doi1, x) < 0)
...
{ //Neu truyen vao con tro -> thay doi ca tree. Neu chi truyen vao root bthg -> phai return de lay gia tri
    ...
    else if (strcasecmp(x.doi1, (*root)->element.doi1) <= 0) //Neu khoa them vao < khoa nut goc
        insertNode(x, &(*root)->left);
    else if (strcasecmp(x.doi1, (*root)->element.doi1) > 0)
...
void deleteNode(elementType x, treeType *root)
{
    if (*root != NULL)
    {
        if (strcmp(x.doi1, (*root)->element.doi1) < 0)
        {
            deleteNode(x, &(*root)->left);
        }
        else if (strcmp(x.doi1, (*root)->element.doi1) > 0)
        {
        ...
        { //Neu chi co 1 con phai->cha tro den thang con phai cua con
            *root = (*root)->right;
        }
        else if ((*root)->right == NULL)
        { //Neu chi co 1 con trai
            *root = (*root)->left;
        }
