...
{
    char tu[maxlength];
    char nghia[maxlength];
} dict;
typedef dict elementType;
...
treeType search(char *x, treeType root)
{
    ...
    else if (strcasecmp(root->element.tu, x) == 0)
    { //Neu tim thay
        return root;
    }
    else if (strcasecmp(root->element.tu, x) < 0)
...
{ //Neu truyen vao con tro -> thay doi ca tree. Neu chi truyen vao root bthg -> phai return de lay gia tri
    ...
    else if (strcasecmp(x.tu, (*root)->element.tu) <= 0) //Neu khoa them vao < khoa nut goc
        insertNode(x, &(*root)->left);
    else if (strcasecmp(x.tu, (*root)->element.tu) > 0)
...
void deleteNode(elementType x, treeType *root)
{
    if (*root != NULL)
    {
        if (strcmp(x.tu, (*root)->element.tu) < 0)
        {
            deleteNode(x, &(*root)->left);
        }
        else if (strcmp(x.tu, (*root)->element.tu) > 0)
        {
